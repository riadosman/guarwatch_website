...
def test_pair_valid_code(client):
    from guardwatch_website.relay.main import pairing, manager
    # Kodu önceden üret
    code = pairing.generate_code("device-test")

    with patch("guardwatch_website.relay.main._register_device_in_backend", new_callable=AsyncMock) as mock_reg:
